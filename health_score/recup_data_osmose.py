...
if __name__ == "__main__":
    ...
    myresult = {"country":"colombia*", "class":{}, "class-extend":{}}
    for i in range(1, 9):
        print("--------", i)

        geojson = fetch_osmose_issues(country="colombia*", item=7040, cls=i, use_dev_item="all", )
        # Traitement simple : affichage du nombre de features
        features = geojson.get("features", [])
        print(f"Nombre de features récupérées : {len(features)}")
    ...
    with open("osmose_colombia.json", "w", encoding="utf-8") as f:
