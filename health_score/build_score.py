...
def connectivity_analysis(graph):
    ...
    if len(df_stat) == 0:
        ...
        stats_substation_connectivity_pct = -1
    else:
...
df_power_line = gpd.read_file("data/CO/osm_brut_power_line.gpkg")
df_power_tower = gpd.read_file("data/CO/osm_brut_power_tower_transition.gpkg")
df_power_substation = gpd.read_file("data/CO/osm_clean_power_substation.gpkg")
df_pregraph_power_nodes = gpd.read_file("data/CO/pre_graph_power_nodes.gpkg")
df_pregraph_power_lines = gpd.read_file("data/CO/pre_graph_power_lines.gpkg")
gdf_postgraph_nodes = gpd.read_file("data/CO/post_graph_power_nodes.gpkg")
gdf_postgraph_lines = gpd.read_file("data/CO/post_graph_power_lines.gpkg")

gdf_postgraph_lines_circuit = gpd.read_file("data/CO/post_graph_power_lines_circuit.gpkg")
gdf_postgraph_nodes_circuit = gpd.read_file("data/CO/post_graph_power_nodes_circuit.gpkg")
...
stats["nb_substations"] = len([n for n in G.nodes if G.nodes[n]["grid_role"] == "substation"])
...
with open("osmose_colombia.json") as f:
    data = json.load(f)
...
key = "grid_connectivity_without_circuit"
...
key = "grid_connectivity_with_circuit"
...
for key in indicators.keys():
    output_data.append({"name":names.get(key, ""), "value":indicators_p.get(key, ""), "explain":explanations.get(key, "")})
